...
function getDateAndDay() {
	...
	for (let i = 1; i <= endDate.getDate(); i++) {
		...
		const result = jsValue.find(({clockInTime}) => startDate.getTime() <= clockInTime && endDate.getTime() >= clockInTime);
		console.log(result);
		...
		if(result !== undefined){
			...
			console.log("now: ", result.clockInTime/1000, now.getHours(), now.getMinutes(), now.getSeconds())
